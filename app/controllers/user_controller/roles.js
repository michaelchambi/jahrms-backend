...
const submodule_item = db.app_submodule_item;
const sub_module_item_permission = db.app_sub_module_item_permission;
const sub_module_item_action = db.app_sub_module_item_action;
...
exports.add = (req, res) => {
	...
	roles.create({
		...
		app_modules.findAll().then(module => {
			for (const key in module) {
				app_module_permission.create({
					permission: false,
			...
			app_sub_modules.findAll().then(submodule => {
				for (const key in submodule) {
					app_sub_module_permission.create({
						permission: false,
				...
				app_sub_module_action.findAll().then(action => {
			...
			}).then(() => { // submodule item creation


				submodule_item.findAll().then(submodule_items => {
					for (const key in submodule_items) {
						sub_module_item_permission.create({
							permission: false,
							sub_module_item_id: submodule_items[key].id,
							role_id: role.id
						}).catch(err => {
							res.status(500).json({
								en_message: "Fail to add sub module Item permission",
								sw_message: "Imeshwindwa kuongeza majukumu ya module ndogo"
							});
						});
					}
				}).then(() => {
					sub_module_item_action.findAll().then(item_action => {
						for (const key in item_action) {
							app_action_permission.create({
								permission: false,
								action_id: item_action[key].id,
								role_id: role.id
							}).catch(err => {
								res.status(500).json({
									en_message: "Fail to add sub module action permission",
		...
		res.status(500).json({
			en_message: err+"Fail to add roles",
